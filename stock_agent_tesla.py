...
_auth = open(_token_file).read().strip() if _token_file else None
client = anthropic.Anthropic(auth_token=_auth) if _auth else anthropic.Anthropic()
TAVILY_KEY = os.environ.get("TAVILY_API_KEY")
tavily = TavilyClient(api_key=TAVILY_KEY) if TAVILY_KEY else None

tools = [{"name": "web_search", "description": "Search web for stock prices, news, analyst ratings.",
          "input_schema": {"type": "object", "properties": {"query": {"type": "string"}}, "required": ["query"]}}]

MOCK = json.dumps([
    {"title": "Tesla TSLA $248.71 +2.3%", "content": "TSLA at $248.71. P/E 72. Avg analyst target $275. Consensus: BUY (18 buy, 9 hold, 6 sell). 52-wk range $138–$300.", "url": "https://finance.yahoo.com/quote/TSLA"},
...
def web_search(query):
    ...
    print("   ⚠️  No TAVILY_API_KEY — using mock data")
    return MOCK
